...
// const GMGN_API_URL = 'https://gmgn.ai/defi/router/v1/sol'; 
...
//     async checkLimit(key, maxRequests = 10, timeWindow = 1000) { // Default to 10 req/sec
...
//             console.log(`[RateLimiter] Limit for ${key} reached. Waiting ${waitTime}ms...`);
//             await new Promise(res => setTimeout(res, waitTime));
//             // After waiting, reset the count and time for the next window
//             limit.count = 0;
//             limit.resetTime = Date.now() + timeWindow;
//         }
...
//                 console.log(`[Token Discovery] Found ${candidates.length} recent candidates from DexScreener.`);
//                 return candidates;
//             }
...
//     // ... other functions like sendPnLChart, autoSell, etc. remain the same ...
...
//         try {
//             await this.heliusRateLimiter.checkLimit('jupiter', 30, 1000);
...
//                 overallScore,
//                 recommendation,
//                 analysis: { security, liquidity, whale, sentiment, volatility }
//             };
...
//         await this.heliusRateLimiter.checkLimit('helius_general');
//         const HELIUS_API_KEY = process.env.HELIUS_API_KEY;
...
//                 params: { mint: tokenAddress, limit: 100 }
//             });
...
//             if (accounts.length === 0) return { score: 0, reason: 'No token holders found' };
...
//             const concentration = totalAmount > 0 ? maxHolding / totalAmount : 1;
...
//             else if (concentration > 0.8) score -= 25;
...
//         try {
//             const params = { id: tokenAddress, limit };
...
//                 await this.heliusRateLimiter.checkLimit('helius_general'); // Rate limit each batch
//                 const response = await axios.post(url, {
//                     jsonrpc: '2.0',
...
//             return {
//                 score: Math.max(0, Math.min(100, score)),
//                 reason: `Security analysis complete (${positiveFactors.length}p, ${negativeFactors.length}n)`,
//                 positiveFactors,
//                 negativeFactors,
//                 metadata
//             };
...
//             if (transactions.length < 10) return { score: 40, reason: 'Low recent transaction volume' };
...
//             if (amounts.length === 0) return { score: 30, reason: 'No recent transfers found' };
    
//             const avgAmount = totalVolume / amounts.length;
//             const maxAmount = Math.max(...amounts);
//             let score = 70;
            
//             if (maxAmount > avgAmount * 10) score -= 20;
//             const largeTransfers = amounts.filter(amt => amt > avgAmount * 3).length;
//             if (largeTransfers > 5) score -= 15;
...
//             if (currentCount > previousCount * 1.5) score += 30;
//             else if (currentCount < previousCount * 0.5) score -= 20;
...
//             if (avgAbsChange > 50) { score = 10; riskLevel = 'EXTREME'; } 
//             else if (avgAbsChange > 25) { score = 30; riskLevel = 'HIGH'; } 
//             else if (avgAbsChange > 10) { score = 60; riskLevel = 'MEDIUM'; } 
//             else { score = 90; riskLevel = 'LOW'; }
            
//             return { score, volatility: avgAbsChange, riskLevel, priceChanges };
//         } catch (error) {
//             return { score: 0, volatility: 999, riskLevel: 'EXTREME' };
//         }
//     } 
...
//             '9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM', '14ivtgssEBoBjuZJtSAPKYgpUK7DmnSwuPMqJoVTSgKJ',
//             '9W959DqEETiGZocYWCQPaJ6sBmUzgfxXfqGeTEdp3aQP', '11111111111111111111111111111112',
//             'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA', 'JUP2jxvXaqu7NQY1GmNF4m1vodw12LVXYxbFL2uJvfo',
//             'JUP3c2Uh3WA4Ng34tw6kPd2G4C5BB21Xo36Je1s32Ph', 'GThUX1Atko4tqhN2NaiTazWSeFWMuiUirestXkAtVyiS',
...
//         try {
//             if (!this.marketSentiment) return true;
...
//         try {
//             let slippageBps = Math.floor(MAX_SLIPPAGE * 10000);
            
//             if (volatilityLevel > 2.0) slippageBps = 200;
//             else if (volatilityLevel < 1.0) slippageBps = 50;
...
//             case 'HIGH': multiplier = 0.7; break;
...
//         return Math.max(baseAmount * multiplier, baseAmount * 0.3);
//     }  
    
//     calculateDynamicTarget(entryPrice, analysis) {
//         let targetMultiplier = 1.25;
        
//         const volatility = analysis.analysis.volatility.volatility;
//         const overallScore = analysis.overallScore;
        
//         if (volatility < 1.0) targetMultiplier = 1.30;
//         else if (volatility > 2.0) targetMultiplier = 1.20;
        
//         if (overallScore > 80) targetMultiplier += 0.05;
...
//         if (volatility > 2.0) stopLossMultiplier = 0.92;
//         else if (volatility < 1.0) stopLossMultiplier = 0.97;
        
//         if (securityScore < 60) stopLossMultiplier -= 0.02;
...
//             if (transactions.length < 10) return { suspicious: false };
    
//             let largeSellsToExchange = 0;
//             let failedTxCount = 0;
    
//             for (const tx of transactions) {
//                 if (tx.meta && tx.meta.err !== null) failedTxCount++;
...
//             if (largeSellsToExchange > 3) return { suspicious: true, reason: 'Large sell-offs to exchanges detected' };
            
//             const failureRate = failedTxCount / transactions.length;
//             if (failureRate > 0.3) return { suspicious: true, reason: 'High transaction failure rate detected' };
...
//             else if (days > 30) return `${Math.floor(days / 30)} months`;
...
//             if (tokenData.price < 0.000001) { score += 20; notes.push('Extremely low price'); }
//             if (tokenData.volume24h < 10000) { score += 15; notes.push('Low trading volume'); }
//             if (tokenData.marketCap < 100000) { score += 20; notes.push('Very low market cap'); }
//             if (tokenData.liquidity && tokenData.liquidity < 50000) { score += 15; notes.push('Low liquidity'); }
...
//                 return { success: true, transaction, quote, tokenData };
//             } catch (error) {
...
//             if (transaction.success) userState.successfulTrades++;
...
//             try {
//                 const marketData = { totalVolume: 0, gainers: 0, losers: 0, neutral: 0 };
...
//                 this.marketSentiment = { sentiment, volume: marketData.totalVolume, timestamp: Date.now(), ...marketData };
//             } catch (error) {
...
//                 try {
//                     if (action.startsWith('buy_')) await this.handleBuyAction(chatId, userId, action.replace('buy_', ''));
//                     else if (action.startsWith('sell_')) await this.handleSellAction(chatId, userId, action.replace('sell_', ''));
//                     else if (action.startsWith('alert_')) await this.handleAlertAction(chatId, userId, action.replace('alert_', ''));
//                     else if (action.startsWith('watch_')) await this.handleWatchlistAction(chatId, userId, action.replace('watch_', ''));
...
//                 await this.bot.sendMessage(msg.chat.id, welcomeMessage, { parse_mode: 'Markdown', disable_web_page_preview: true });
//             });
    
//             this.bot.onText(/\/status/, (msg) => this.isAuthorized(msg.from.id) && this.showStatus(msg.chat.id, msg.from.id));
//             this.bot.onText(/\/start_plan/, (msg) => this.isAuthorized(msg.from.id) && this.startTradingPlan(msg.chat.id, msg.from.id));
//             this.bot.onText(/\/search (.+)/, (msg, match) => this.isAuthorized(msg.from.id) && this.searchToken(msg.chat.id, match[1]));
...
//                 await this.bot.sendMessage(msg.chat.id, success ? 'helius integration working!' : 'helius integration failed');
//             });
//             this.bot.onText(/\/trade (\w+) ([\d.]+) (buy|sell)/, (msg, match) => this.isAuthorized(msg.from.id) && this.handleTradeCommand(msg.chat.id, msg.from.id, match[1], parseFloat(match[2]), match[3]));
//             this.bot.onText(/\/pause/, async (msg) => {
//                 if (!this.isAuthorized(msg.from.id)) return;
//                 this.getUserState(msg.from.id).isActive = false;
//                 await this.saveUserStates();
...
//                     text += `*${i + 1}. ${pos.symbol}*\n🟢 Entry: $${pos.entryPrice.toFixed(4)}\n🎯 Target: $${pos.targetPrice.toFixed(4)}\n🛑 Stop-Loss: $${pos.stopLossPrice.toFixed(4)}\n📅 Bought: ${new Date(pos.boughtAt).toLocaleString()}\n\n`;
//                 });
//                 await this.bot.sendMessage(msg.chat.id, text, { parse_mode: 'Markdown', disable_web_page_preview: true });
//             });
        
//             this.bot.onText(/\/watchlist/, (msg) => this.isAuthorized(msg.from.id) && this.showWatchlist(msg.chat.id, msg.from.id));
//             this.bot.onText(/\/alerts/, (msg) => this.isAuthorized(msg.from.id) && this.showAlerts(msg.chat.id, msg.from.id));
//             this.bot.onText(/\/settings/, (msg) => this.isAuthorized(msg.from.id) && this.showSettings(msg.chat.id, msg.from.id));
//             this.bot.onText(/\/setalert (\w+) (above|below) ([\d.]+)/, (msg, match) => this.isAuthorized(msg.from.id) && this.setAlert(msg.chat.id, msg.from.id, match[1], match[2], parseFloat(match[3])));
//             this.bot.onText(/\/market/, (msg) => this.isAuthorized(msg.from.id) && this.showMarketOverview(msg.chat.id));
//             this.bot.onText(/\/help/, (msg) => this.isAuthorized(msg.from.id) && this.showHelp(msg.chat.id));
//             this.bot.onText(/\/pnl/, (msg) => this.isAuthorized(msg.from.id) && this.sendPnLChart(msg.chat.id)); 
...
//                 const alert = { id: alertId, token: token.toUpperCase(), type, price, createdAt: Date.now() };
                
//                 if (!this.priceAlerts.has(userId)) this.priceAlerts.set(userId, []);
//                 this.priceAlerts.get(userId).push(alert);
                
//                 await this.bot.sendMessage(chatId, `🔔 Alert set for ${token.toUpperCase()} ${type} ${price.toFixed(6)}`);
//             } catch (error) {
...
const MAX_SLIPPAGE = 0.01; // 1% maximum slippage
const DEFAULT_STOP_LOSS = 0.05; // 5% stop loss
const JUPITER_API_URL = 'https://quote-api.jup.ag/v6'; // Kept for reference, but not used for swaps
const RAYDIUM_API_URL = 'https://api.raydium.io/v2';
const COINGECKO_API_URL = 'https://api.coingecko.com/api/v3';
const DEXSCREENER_API_URL = 'https://api.dexscreener.com/latest/dex';
const GMGN_API_URL = 'https://gmgn.ai/defi/router/v1/sol'; // ++ ADDED: New API endpoint for swaps
...
class TradingBot {
    ...
    // ** MODIFIED & RENAMED to be more robust and accurate
    async discoverNewTokens() {
        console.log('[Token Discovery] Fetching recent pairs from DexScreener...');
        try {
            const response = await axios.get(`${DEXSCREENER_API_URL}/pairs/solana/new`, {
                 // Using the '/new' endpoint can be more reliable for discovery
                timeout: 15000
            });
            ...
            if (response.data && response.data.pairs) {
                const candidates = response.data.pairs
                    .filter(pair => {
                        ...
                        const volume = pair.volume?.h24 || 0;
                        const ageMinutes = (Date.now() - (pair.pairCreatedAt || 0)) / 60000;
                        // Ensure it's a SOL or USDC pair for reliable pricing
                        const isSolOrUsdcPair = [COMMON_TOKENS.SOL, COMMON_TOKENS.USDC].includes(pair.quoteToken.address);

                        return (
                            isSolOrUsdcPair &&
                            liquidity > 5000 &&
                            volume > 1000 &&
                            ageMinutes > 10 &&  // At least 10 minutes old
                            pair.baseToken.symbol &&
                            pair.baseToken.address
                        );
                    })
                    .slice(0, 30) // Take the top 30 candidates
                    .map(pair => ({
                        address: pair.baseToken.address,
                        symbol: pair.baseToken.symbol,
                        name: pair.baseToken.name
    ...
    // ... other functions like sendPnLChart, etc. remain the same ...
    async sendPnLChart(chatId) {
        const width = 800;
        const height = 400;
        const tradesFile = 'live_trades.json';
    ...
    // ** MODIFIED to use gmgn.ai for selling
    async autoSell(userId, pos, exitPrice, reason) {
        try {
            const user = this.getUserState(userId);
            let earned = 0;
            ...
            if (LIVE_TRADING) {
                const sellTxData = await this.getGmgnSwapTx(
                    pos.tokenAddress,
                    COMMON_TOKENS.USDC,
                    pos.tokensOwned // The amount is already in lamports from the buy tx
                );

                if (sellTxData) {
                    const tx = await this.executeSwapTransaction(sellTxData.tx);
                    if (tx.success) {
                        earned = sellTxData.out_amount / (10**6); // Convert USDC back to dollars
                        console.log(`✅ Sold ${pos.symbol} | TX: ${tx.signature}`);
                    } else {
                        console.log(`❌ Sell failed for ${pos.symbol}: ${tx.error}`);
                        return;
                    ...
                    console.log(`❌ No gmgn.ai sell route for ${pos.symbol}`);
                    return;
                }
            } else {
                // Simulation logic
                earned = (pos.tokensOwned / (10**6)) * exitPrice; // Approximate sell value
            }
    ...
    // ++ NEW FUNCTIONS to handle gmgn.ai API calls and execution
    // ++ This function gets a swap transaction from gmgn.ai
    async getGmgnSwapTx(inputMint, outputMint, amount, slippageBps = 50) { // Default slippage 0.5%
        if (!this.wallet) return null;
        try {
            const url = `${GMGN_API_URL}/tx/get_swap_route`;
            const response = await axios.get(url, {
                params: {
                    ...
                    slippage: slippageBps / 10000, // gmgn uses decimal for slippage
                },
                timeout: 15000 // 15-second timeout
            });

            if (response.data && response.data.tx) {
                return response.data; // This contains quote info and the base64 tx
            }
            return null;

        } catch (error) {
            console.error(`❌ gmgn.ai quote error: ${error.response?.data?.message || error.message}`);
            return null;
        }
    }

    // ++ This function executes any base64 swap transaction
    async executeSwapTransaction(base64Tx) {
        try {
            if (!this.wallet) throw new Error('Wallet not configured for execution');

            const transactionBuf = Buffer.from(base64Tx, 'base64');
            const transaction = Transaction.from(transactionBuf);
            transaction.sign(this.wallet);
    ...
    async testHeliusConnection() {
        ...
        try {
            // This now tests the new token discovery function
            const candidates = await this.discoverNewTokens();
            console.log(`✅ Token discovery: Found ${candidates.length} candidates`);
    ...
    async simulateSwap(route) {
        try {
            // Note: This function was Jupiter-specific. gmgn.ai does not have a separate simulate endpoint.
            // The main `getGmgnSwapTx` call can be considered the "quote" and is sufficient.
            // This function may no longer be needed or can be adapted if a simulation feature is found.
            console.log("Note: gmgn.ai does not support a separate simulation endpoint like Jupiter.");
            return { success: true, data: "Simulation not applicable for gmgn.ai" };
        } catch (error) {
            return { success: false, error: error.message };
        }
    }
    ...
    // ** MODIFIED to use gmgn.ai and new discovery function
    async autoBuyTokenIfEligible(userId) {
        try {
            const user = this.getUserState(userId);
            const dayIndex = user.currentDay - 1;
            ...
            const candidates = await this.discoverNewTokens(); // Using the new function
            if (!candidates.length) {
                console.log('[AutoBuy] FAILED: No initial candidates found from Token Discovery.');
                return;
            }
            ...
            // Use gmgn.ai to get the swap transaction
            const amountInLamports = Math.floor(adjustedAmount * (10**6)); // USDC has 6 decimals
            const swapData = await this.getGmgnSwapTx(
                COMMON_TOKENS.USDC,
                selected.address,
                amountInLamports
            );
            ...
            if (!swapData) {
                console.log(`[AutoBuy] FAILED: No gmgn.ai route found for ${selected.symbol}.`);
                return;
            }
            console.log(`[AutoBuy] Successfully received a trade quote from gmgn.ai for ${selected.symbol}.`);

            const tokensReceived = swapData.out_amount;
            const usdcSpent = swapData.in_amount / (10**6);
            // Attempt to get decimals from metadata, fallback to 6
            const decimals = selected.analysis.analysis.security.metadata?.supply?.decimals || 6;
            const entryPrice = usdcSpent / (tokensReceived / (10**decimals));

            if (LIVE_TRADING) {
                console.log(`[AutoBuy] LIVE TRADING IS ON. Attempting to execute swap for ${selected.symbol}...`);
                const tx = await this.executeSwapTransaction(swapData.tx); // Use the new generic executor
                if (!tx.success) {
                    console.log(`[AutoBuy] TRANSACTION FAILED for ${selected.symbol}: ${tx.error}`);
                    await this.bot.sendMessage(userId, `❌ Trade execution failed for ${selected.symbol}: ${tx.error}`);
                    return;
            ...
            const position = {
                ...
                stopLossPrice: this.calculateDynamicStopLoss(entryPrice, selected.analysis),
                amountUSD: adjustedAmount,
                tokensOwned: tokensReceived, // This is in the smallest unit (lamports)
                boughtAt: Date.now(),
                status: 'open',
                analysis: selected.analysis,
                riskLevel: selected.analysis.analysis.volatility.riskLevel,
            ...
            console.error(`[AutoBuy] CRITICAL ERROR in auto-buy process for user ${userId}:`, error.message, error.stack);
            await this.bot.sendMessage(userId, `❌ A critical error occurred during the auto-buy process: ${error.message}`);
        }
    }
    ...
    🪙 Token: ${position.symbol}
    💰 Amount: $${position.amountUSD.toFixed(2)}
    📊 Tokens Received (raw): ${position.tokensOwned.toLocaleString()}
    ...
    // ** MODIFIED to get a live price using gmgn.ai
    async getLivePrice(tokenAddress) {
        try {
            // Quote selling 1 token (with appropriate decimals) to USDC
            // This is a common way to get a spot price. We assume 6 decimals for most meme coins,
            // but a more robust solution would fetch decimals from metadata.
            const oneToken = 1 * (10 ** 6);

            const quote = await this.getGmgnSwapTx(
                tokenAddress,
                COMMON_TOKENS.USDC,
                oneToken
            );

            if (quote && quote.out_amount) {
                const price = quote.out_amount / (10 ** 6); // USDC also has 6 decimals
                return price;
            }
            return null;
        } catch (error) {
            console.error(`❌ Price fetch failed for ${tokenAddress}: ${error.message}`);
            return null;
        }
    }
    ...
    // ** MODIFIED to use the new getLivePrice function
    async monitorAndSell() {
        for (const [userId, user] of this.userStates.entries()) {
            for (const pos of user.positions.filter(p => p.status === 'open')) {
                try {
                    const currentPrice = await this.getLivePrice(pos.tokenAddress);
    ...
    **Age**: ${tokenData.age || 'Unknown'}
    ...
    async executeTrade(userId, tokenSymbol, amount, side) {
        // This function is now a manual trade wrapper around the gmgn.ai functions
        try {
            if (!this.wallet) throw new Error('Wallet not configured');
            ...
            if (side === 'buy') {
                ...
                amountInLamports = Math.floor(amount * (10**6)); // amount is in USD
            } else { // sell
                inputMint = tokenData.address;
                outputMint = COMMON_TOKENS.USDC;
                // This part is tricky for manual sell without knowing token decimals. Assuming 6.
                amountInLamports = Math.floor(amount * (10**6)); // amount is in number of tokens
            }

            const swapData = await this.getGmgnSwapTx(inputMint, outputMint, amountInLamports);
            if (!swapData) throw new Error('Unable to get a quote from gmgn.ai');

            const transaction = await this.executeSwapTransaction(swapData.tx);

            // Manual trading doesn't automatically update state like auto-trading does
            // This part can be enhanced to create/close positions for manual trades
            await this.updateUserStateAfterTrade(userId, tokenData, amount, side, transaction);

            return { success: transaction.success, transaction, tokenData };
        } catch (error) {
            console.error('❌ Trade execution error:', error);
            return { success: false, error: error.message };
        }
    ...
    **🔥 NEW FEATURES:**
    ✅ Real-time price monitoring
    ✅ Actual Solana blockchain trading via gmgn.ai
    ✅ Stop-loss protection
    ✅ Risk assessment
    ✅ Market analysis
    ✅ Price alerts
    ...
    **Commands:**
    ...
    async handleTradeCommand(chatId, userId, token, amount, side) {
        try {
            ...
            await this.bot.sendMessage(chatId, `🔄 Executing ${side} order for ${amount} ${side === 'buy' ? 'USD of' : ''} ${token}...`);
